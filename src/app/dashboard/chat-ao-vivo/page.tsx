        ...
        <div className="space-y-2">
          {conversations.map((conv) => {
            ...
            return (
              <div
                key={conv.id}
                className="flex items-center gap-4 rounded-xl border border-gray-800 bg-gray-900 p-4 hover:border-gray-700 transition cursor-pointer"
              >
                <div className="w-10 h-10 rounded-full bg-green-600/20 flex items-center justify-center text-green-500 font-bold text-sm">
                  {customer?.name?.charAt(0)?.toUpperCase() || "?"}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between">
                    <p className="font-medium truncate">{customer?.name || customer?.phone || "Desconhecido"}</p>
                  ...
                  <p className="text-sm text-gray-400 truncate">{conv.last_message || "Nova conversa"}</p>
                </div>
