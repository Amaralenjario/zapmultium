...
export default function DashboardFilter() {
  ...
  const applyFilter = (range: string, start?: string, end?: string) => {
      ...
      {showCustom && (
        <div className="flex items-center gap-1.5 ml-1">
          <input
            ...
            className="text-[11px] rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 px-2.5 py-1.5 text-gray-700 dark:text-gray-300 focus:ring-1 focus:ring-emerald-500 focus:border-emerald-500 outline-none"
          />
          ...
          />
          <button
            onClick={() => applyFilter("custom", startDate, endDate)}
            disabled={!startDate || !endDate}
            className="text-[11px] font-medium px-3 py-1.5 rounded-lg bg-emerald-500 text-white disabled:opacity-40 transition"
          >
            Aplicar
          </button>
        </div>
